		...
		public ArrayList<Pedido> pedidosEntreFecha(LocalDate f1, LocalDate f2) {
			...
			try(FileReader fr = new FileReader(fichero); BufferedReader bf = new BufferedReader(fr)) {
				...
				while(linea!=null) {
					...
					if((pedido.fechaPedido().compareTo(f1) >= 0 && pedido.fechaPedido().compareTo(f2) <= 0)) {
		...
		public Pedido pedidoProximoFecha(LocalDate fecha) {			
			...
			try(FileReader fr = new FileReader(fichero); BufferedReader bf = new BufferedReader(fr)) {
				...
				while(linea!=null) {
					...
					if(Math.abs(ChronoUnit.DAYS.between(pedido.fechaPedido(), fecha)) > Math.abs(ChronoUnit.DAYS.between(pedido2.fechaPedido(), fecha))){				
						pedido = pedido2;
					}
					linea = bf.readLine();					
				}
